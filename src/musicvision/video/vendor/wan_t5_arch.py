...
# This does NOT match HuggingFace T5EncoderModel — the Wan-AI .pth checkpoints
# use this architecture directly.
import logging
import math
...
def init_weights(m):
    if isinstance(m, T5LayerNorm):
        ...
        if hasattr(m, 'token_embedding') and isinstance(m.token_embedding, nn.Embedding):
...
class T5Encoder(nn.Module):
    ...
    def __init__(self, vocab, dim, dim_attn, dim_ffn, num_heads, num_layers,
                 num_buckets, shared_pos=True, dropout=0.1):
...
def _t5(name, encoder_only=False, return_tokenizer=False,
         tokenizer_kwargs={}, dtype=torch.float32, device='cpu', **kwargs):
    if encoder_only:
        model_cls = T5Encoder
...
class T5EncoderModel:
    """Wan-AI's T5 encoder wrapper — matches their checkpoint format exactly."""
    ...
    def __init__(self, text_len, dtype=torch.bfloat16, device='cpu',
        ...
        self.tokenizer = HuggingfaceTokenizer(
            name=tokenizer_path or 'google/umt5-xxl',
            seq_len=text_len,
            clean='whitespace',
        )
